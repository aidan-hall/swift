...
/// Recursively generate an #if ... #elseif ... #else ... chain of suppressible
/// feature checks for each combination of the features generated by generator.
/// Include the features in enabledFeatures in each #if and #elseif case.
///
/// `generator` is passed by value, so multiple recursive calls at the same
/// level can obtain the same feature by calling `generator.next()`.
static void printWithSuppressibleFeatureChecksRecurse(
    ASTPrinter &printer, const PrintOptions &options, bool firstInChain,
    bool includeCompilerCheck, FeatureSet::SuppressibleGenerator generator,
    llvm::function_ref<void()> printBody, BasicFeatureSet &enabledFeatures) {
  // If the generator has been exhausted, we have reached the recursive base
  // case. Generate an `#if` or `#elseif` block that checks for the
  // currently-enabled set of features, and print the body.
  if (generator.empty()) {

    // If no features are enabled we know we have reached the last combination,
    // due to the order of recursive calls. Emit an `#else` clause.
    if (enabledFeatures.empty()) {
      printer << "#else";
      printer.printNewline();
    } else {
      printCompatibilityCheckIf(printer, /*elseif*/ !firstInChain,
                                includeCompilerCheck, enabledFeatures);
    }
    printBody();
    printer.printNewline();
    return;
  ...
  // Otherwise, make two recursive calls for the next feature: one with it
  // enabled, and one with it disabled.
  Feature feature = generator.next();
  assert(!enabledFeatures.contains(feature));
  
  // Enumerate cases with feature enabled.
  enabledFeatures.insert(feature);
  printWithSuppressibleFeatureChecksRecurse(
    printer, options, firstInChain, includeCompilerCheck, generator, printBody, enabledFeatures);

  // Enumerate cases with feature disabled.
  enabledFeatures.remove(feature);
  suppressingFeature(options, feature, [&] {
    printWithSuppressibleFeatureChecksRecurse(printer, options,
                                       /*first*/ false,
                                       includeCompilerCheck, generator,
                                       printBody, enabledFeatures);
  });
}

/// Generate a #if ... #elseif ... #else ... #endif chain with suppressible feature checks
/// for every combination of the features that the supplied generator will produce.
static void printWithSuppressibleFeatureChecks(
    ASTPrinter &printer, const PrintOptions &options, bool includeCompilerCheck,
    FeatureSet::SuppressibleGenerator generator,
    llvm::function_ref<void()> printBody) {
  assert(!generator.empty());
  BasicFeatureSet enabledFeatures = {};
  printWithSuppressibleFeatureChecksRecurse(printer, options, true,
                                            includeCompilerCheck, generator,
                                            printBody, enabledFeatures);
  // Close the chain.
  printer << "#endif";
