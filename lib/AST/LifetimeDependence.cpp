...
void LifetimeDependenceInfo::Profile(llvm::FoldingSetNodeID &ID) const {
  ID.AddBoolean(addressableParamIndicesAndImmortalAndInferred.getPointer()
                    .getInt()); // isImmortal
  ID.AddBoolean(
      addressableParamIndicesAndImmortalAndInferred.getInt()); // isInferred
  ...
  if (addressableParamIndicesAndImmortalAndInferred.getPointer().getPointer()) {
    ID.AddBoolean(true);
    addressableParamIndicesAndImmortalAndInferred.getPointer()
        .getPointer()
        ->Profile(ID);
  } else {
    ID.AddBoolean(false);  
...
void LifetimeDependenceInfo::getConcatenatedData(
  ...
  if (hasAddressableParamIndices()) {
    pushData(addressableParamIndicesAndImmortalAndInferred.getPointer()
                 .getPointer());
  }
}
...
struct LifetimeDependenceBuilder {
  struct TargetDeps {
    ...
    TargetDeps(bool hasAnnotation, unsigned capacity, bool isInferred)
        : inheritIndices(capacity), scopeIndices(capacity),
          hasAnnotation(hasAnnotation), isInferred(isInferred) {}
...
public:
  ...
  TargetDeps *createAnnotatedTargetDeps(unsigned targetIndex) {
    auto iterAndInserted = depsArray.try_emplace(
        targetIndex, true, sourceIndexCap, /*isInferred*/ false);
  ...
  TargetDeps *getInferredTargetDeps(unsigned targetIndex) {
    auto iter = depsArray
                    .try_emplace(targetIndex, false, sourceIndexCap,
                                 /*isInferred*/ true)
                    .first;
  ...
  initializeDependenceInfoArray(ASTContext &ctx) const {
    ...
    for (auto &idxAndDeps : depsArray) {
      ...
      lifetimeDependencies.push_back(LifetimeDependenceInfo(
          ...
          /*isImmortal*/ deps.isImmortal, /*isInferred*/ deps.isInferred));
    }
...
class LifetimeDependenceChecker {
  ...
  struct ParamInfo {
    ...
    SourceLoc loc;
    Type typeInContext;
    ParamInfo(Param param, SourceLoc loc, Type typeInContext)
        : param(param), loc(loc), typeInContext(typeInContext) {}
  };
...
protected:
    ...
    for (auto &paramInfo : parameterInfos) {
      ...
      if (!depBuilder.hasTargetDeps(paramIndex)) {
        ctx.Diags.diagnose(paramInfo.loc, diagID,
                           {StringRef(diagnosticQualifier()),
                            paramInfo.param.getInternalLabel().str()});
  ...
  getDependenceKindFromDescriptor(LifetimeDescriptor descriptor,
    ...
    Param const &param = paramInfo.param;
    auto const loc = descriptor.getLoc();
    auto const type = paramInfo.typeInContext;
  ...
  std::optional<std::pair<ParamInfo const *, unsigned>>
  getParamFromDescriptor(LifetimeDescriptor descriptor) {
    ...
    case LifetimeDescriptor::DescriptorKind::Named: {
      ...
      return std::make_pair(candidate, candidate - parameterInfos.begin());
    }
  ...
  void initializeAttributeDeps() {
    for (LifetimeEntry *entry : lifetimeEntries) {
      ...
      if (targetDescriptor.has_value()) {
        ...
        auto *paramInfo = targetParamAndIndex->first;
        targetIndex = targetParamAndIndex->second;
        if (!paramInfo->param.isInOut()) {
          ctx.Diags.diagnose(paramInfo->loc,
                             diag::lifetime_parameter_requires_inout,
                             targetDescriptor->getString());
        }
        if (isDiagnosedEscapable(paramInfo->typeInContext)) {
  ...
  void initializeDescriptorDeps(unsigned targetIndex,
    ...
    auto *paramInfo = paramAndIndex->first;
    unsigned sourceIndex = paramAndIndex->second;
    auto lifetimeKind = getDependenceKindFromDescriptor(source, *paramInfo);
    ...
    for (auto const &paramInfo : parameterInfos) {
      ...
      if (!kind) {
        diagnose(
            ...
            paramInfo.param.getInternalLabel().str(), diagnosticQualifier());
      }
  ...
  void inferNonEscapableResultOnParam() {
    ...
    auto const &param = paramInfo.param;
    Type paramTypeInContext = paramInfo.typeInContext;
    if (paramTypeInContext->hasError()) {
      return;
    }
  ...
  for (auto source : lifetimeDependentRepr->getLifetimeEntry()->getSources())
  {
    ...
    case LifetimeDescriptor::DescriptorKind::Named: {
      ...
      return LifetimeDependenceInfo(/*inheritLifetimeParamIndices*/ nullptr,
                                    ...
                                    targetIndex,
                                    /*isImmortal*/ true, /*isInferred*/ false);
    }
    default:
