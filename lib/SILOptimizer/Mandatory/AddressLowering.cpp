...
struct AddressLoweringState {
  ...
  // Loadable, trivial values that were passed as @in_guaranteed in indirect
  // applies mapped to their associated (temporary) storage. This is a cache so
  // these temporary allocations can be reused.
  //
  // Only values with trivial types are stored because non-trivial types must be
  // borrowed. Borrowing the value for the duration of the function could cause
  // conflicts with other uses.
  llvm::SmallDenseMap<SILValue, AllocStackInst*> nonopaqueArgumentValueStorageMap;

  SILBuilder nonopaqueArgumentAllocBuilder;
  ...
  AddressLoweringState(SILFunction *function, DominanceInfo *domInfo,
      ...
      : function(function), loweredFnConv(getLoweredFnConv(function)),
        domInfo(domInfo), nonopaqueArgumentAllocBuilder(function->getEntryBlock()->begin()), SLA(SLA) {
...
void CallArgRewriter::rewriteIndirectArgument(Operand *operand) {
  ...
  // Allocate temporary storage for a loadable operand, as appropriate.
  if (apply.getCaptureConvention(*operand).isOwnedConventionInCaller()) {
    ...
    bool canUseTopLevelCachedAllocation =
      !apply.getCaptureConvention(*operand).isInoutConvention()
      && !argValue->getType().isTrivial(pass.function);

    SILValue operandAddr;

    if (canUseTopLevelCachedAllocation) {
      // Reuse or create cached operand address.
      if (pass.nonopaqueArgumentValueStorageMap.contains(argValue)) {
        operandAddr = pass.nonopaqueArgumentValueStorageMap.at(argValue);
      } else {
        // Copy argValue to a temporary allocation that can be reused.
        AllocStackInst *allocInst =
          pass.nonopaqueArgumentAllocBuilder.createAllocStack(RegularLocation::getAutoGeneratedLocation(), argValue->getType());
        pass.nonopaqueArgumentValueStorageMap.insert({argValue, allocInst});

        // The allocations are created at the start of the entry block. If the
        // value is a function argument, insert the store after the allocation.
        // Otherwise, insert the store after the value is introduced.
        SILBuilder builder = [&] {
          auto *definingInsertionPoint = argValue->getDefiningInsertionPoint();
          // TODO: When can a value not have a defining insertion point?
          assert(definingInsertionPoint && "Value has a defining insertion point.");
          if (definingInsertionPoint->getParent() == pass.function->getEntryBlock()
              && isa<SILArgument>(argValue))
            return pass.nonopaqueArgumentAllocBuilder;
          return SILBuilder(definingInsertionPoint);

          return SILBuilder(argValue->getDefiningInstruction());
        }();

        builder.emitStoreValueOperation(builder.getInsertionPointLoc(),
                                        argValue, allocInst,
                                        StoreOwnershipQualifier::Trivial);

        llvm::SmallVector<SILBasicBlock*, 2> exitingBlocks;
        pass.function->findExitingBlocks(exitingBlocks);
        for (auto *block : exitingBlocks) {
          SILBuilder deallocBuilder(std::prev(block->end()));
          deallocBuilder.createDeallocStack(RegularLocation::getAutoGeneratedLocation(), allocInst);
        }

        operandAddr = allocInst;
      }
    } else {
      AllocStackInst *allocInst = argBuilder.createAllocStack(callLoc, argValue->getType());
